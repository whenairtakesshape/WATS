...
const CompareCitiesRoute = () => {
    ...
    const [breathingStage, setBreathingStage] = useState<CompareCitiesBreathingStage>(CompareCitiesBreathingStage.NOT_BREATHING);
   ...
   */
    const handleBreatheButtonClick = async () => {
        if (selectedCity) {
            try {
                const res = await axios.post(`http://localhost:3001/aqi?value=${vancouverData.aqiRating}`);
                console.log(res);
